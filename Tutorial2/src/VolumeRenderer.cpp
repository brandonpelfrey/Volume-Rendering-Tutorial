...
void VolumeRenderer::render(const Vec3& cameraPosition, const Vec3& cameraFocus, float marchDistance, int marchingStepCount, const Grid& dsm) {
	...
	for(int px=0;px<width;++px) {
		for(int py=0;py<height;++py) {
				...
				for(float s=0.f; s<marchDistance; s += ds) {
					...
					float lightValue = dsm.read(rayPosition);
